...
1. The `useChat` composable API changed: `input` and `handleSubmit` were removed in favor of `sendMessage`, and `messages` is now a ref you set directly:
...
1. Replace `<MDC>` with `<Markdown>` for parts-based rendering. The `Markdown` component is purpose-built for streaming Markdown, it incrementally renders tokens as they arrive from the AI. Use AI SDK helpers and the `isPartStreaming` utility from `@nuxt/ui/utils/ai`.
...
> [!NOTE]
> See: https://ai-sdk.dev/docs/migration-guides/migration-guide-7-0
> 
> For more details on the AI SDK changes, review the **official AI SDK migration guides**.

> [!TIP]
> See: https://github.com/nuxt/ui/pull/6636
> 
> View all changes in the **AI SDK v7 upgrade PR** for a detailed migration reference.

---

- [Migration to v3](https://ui.nuxt.com/docs/getting-started/migration/v3)
